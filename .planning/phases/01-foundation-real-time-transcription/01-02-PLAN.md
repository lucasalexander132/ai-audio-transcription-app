...
must_haves:
  ...
  artifacts:
    ...
    - path: "convex/deepgram.ts"
      provides: "Server-side Deepgram transcription action using REST prerecorded API"
  ...
  <action>
    ...
    3. Create audio recorder hook (`app/lib/hooks/use-audio-recorder.ts`):
       - On startRecording():
         ...
         d. Create MediaRecorder with detected MIME type, timeslice: 2000 (2-second chunks — longer
            chunks give Deepgram more audio context per request, improving diarization quality)
         e. On ondataavailable: validate blob size > 44 bytes (iOS bug check), convert to ArrayBuffer,
            call Convex action (deepgram.transcribeChunk) with transcriptId + audio bytes + mimeType
       ...
       - On stopRecording():
         ...
         c. Combine all collected chunks into single Blob
         d. Upload final recording to Convex file storage:
            1) Call recordings.generateUploadUrl mutation to get an upload URL
            2) POST the audio Blob to the returned upload URL via fetch
            3) The fetch response returns a JSON body with storageId
            4) Call recordings.saveRecording mutation with { transcriptId, storageId, format: detectedMimeType, size: blob.size }
         e. Call transcripts.complete mutation with duration (from elapsed timer)
         f. Update store: status="stopped"
         g. Return transcriptId for navigation to transcript view
    ...
    4. Create Deepgram transcription action (`convex/deepgram.ts`):

       Architecture: Convex action using Deepgram REST prerecorded API.
       Convex actions are stateless (no persistent connections), so we use the synchronous prerecorded
       endpoint for each audio chunk. This is simpler and works naturally with Convex's execution model.

       Implementation — single exported action:
       ```
       export const transcribeChunk = action({
         ...
         handler: async (ctx, args) => {
           ...
           // Call Deepgram prerecorded REST API
           ...
           if (!response.ok) {
             ...
             return; // Silently skip failed chunks rather than crash recording
           }
       ...
       No other actions needed. No WebSocket proxy. No Next.js API route. Just this single
       Convex action that calls Deepgram's REST endpoint per chunk.

       DIARIZATION LIMITATION: Each 2-second chunk is diarized independently by Deepgram.
       Speakers may be labeled inconsistently across chunks (e.g., the same person might be
       "Speaker 0" in one chunk and "Speaker 1" in another). This is acceptable for Phase 1.
       If diarization quality needs improvement, a future phase can add a Next.js WebSocket
       proxy that maintains a persistent Deepgram streaming connection with continuous speaker
       context. For now, the 2-second chunk size gives Deepgram enough audio per request to
       detect multiple speakers within each chunk.

    5. Create recordings storage (`convex/recordings.ts`):
       - `generateUploadUrl` mutation: calls ctx.storage.generateUploadUrl() and returns the URL.
         The client will POST the audio blob directly to this URL, which returns a storageId.
       - `saveRecording` mutation: accepts { transcriptId: v.id("transcripts"), storageId: v.id("_storage"), format: v.string(), size: v.number() }.
         Inserts a record into the recordings table. Checks user authentication.
       - `getRecordingUrl` query: accepts { transcriptId: v.id("transcripts") }, looks up the recording
         by transcriptId index, returns ctx.storage.getUrl(storageId) or null if not found.
       Ensure all mutations/queries check user authentication via ctx.auth.getUserIdentity().
  </action>
  <verify>
    - `npx convex dev --once` deploys successfully (schema + functions)
    - `npm run build` succeeds
    - No TypeScript errors in hook files
    - Deepgram action (transcribeChunk) is importable from convex/_generated/api
    - recordings.generateUploadUrl and recordings.saveRecording are importable from convex/_generated/api
  </verify>
  <done>
    - Audio recorder hook handles start/pause/resume/stop with format fallback
    - Deepgram transcribeChunk action transcribes audio chunks server-side via REST prerecorded API
    - Recording timer tracks elapsed time accurately
    - Upload flow uses generateUploadUrl -> fetch POST -> saveRecording (correct 3-step pattern)
...
5. Audio recording blob is saved to Convex storage via generateUploadUrl -> POST -> saveRecording flow
...
- TRX-02: Speakers automatically detected and labeled (Speaker 1, Speaker 2) — note: cross-chunk speaker consistency is a known limitation of per-chunk diarization; acceptable for Phase 1
