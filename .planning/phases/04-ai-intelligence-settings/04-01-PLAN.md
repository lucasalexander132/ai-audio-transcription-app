...
must_haves:
  truths:
    - "aiSummaries table exists with transcriptId, userId, overview, keyPoints, actionItems, generatedAt, model fields"
    - "userSettings table exists with userId, transcriptionLanguage, autoPunctuation fields"
    - "generateSummary action calls Claude API and stores structured result"
    - "getUserSettings query returns defaults when no settings document exists"
    - "upsertSettings mutation creates or updates user settings"
  artifacts:
