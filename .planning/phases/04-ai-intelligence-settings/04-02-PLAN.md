...
<task type="auto">
  ...
  <action>
    Rewrite the AiSummary component to connect to real backend data. Modify the AiSummaryContent component and the main AiSummary export function.

    **Step 1: Modify AiSummaryContent to remove topics and always show action items section.**

    The existing AiSummaryContent component has a `topics: string[]` parameter and a TOPICS DETECTED section. Per user decisions (CONTEXT.md), only 3 sections are shown: overview, key points, action items. No topics.

    Changes to AiSummaryContent:
    - Remove the `topics` parameter from the props interface entirely
    - Remove the TOPICS DETECTED card section (`{topics.length > 0 && ...}` block)
    - Remove the TopicChip component and TOPIC_STYLES constant (dead code after topics removal)
    - Remove the TagIcon component (dead code after topics removal)
    - Modify the ACTION ITEMS section to ALWAYS render (remove the `{actionItems.length > 0 && ...}` conditional wrapper). When actionItems array is empty, render the SummaryCard with SectionHeader (CheckSquareIcon + "ACTION ITEMS") and a paragraph: "No action items were identified in this transcript." styled with fontSize: 14, color: "#8B7E74", margin: 0, lineHeight: 1.5. When actionItems is non-empty, render the existing ActionItem components as before.

    The updated AiSummaryContent signature should be:
    ```
    ...
    **Step 2: Update the main AiSummary function.**
    ...
    **State A: Summary exists (summary is not null and not undefined)**
    ...
    - No topics parameter needed (removed from AiSummaryContent)
  ...
  <verify>Run `npm run build` or `npx next build` to verify the component compiles without TypeScript errors. Verify that AiSummaryContent no longer accepts a `topics` prop. Verify the ACTION ITEMS section always renders (check for the "No action items" message text in the source). Visually verify in browser: navigate to a completed transcript, click "AI Summary" tab, see the empty state with Generate button.</verify>
  <done>AiSummaryContent renders only 3 sections (overview, key points, action items) with no topics. ACTION ITEMS section always renders -- shows "No action items were identified in this transcript." when empty. AiSummary component queries for existing summary, shows Generate button when none exists, shows skeleton during generation, displays results via AiSummaryContent when complete. Double-tap prevented by disabled state. Error state shown on failure.</done>
</task>
...
- TopicChip, TOPIC_STYLES, and TagIcon removed from the file
- ACTION ITEMS section renders even when actionItems is empty, showing "No action items were identified in this transcript."
...
- Empty action items show "No action items were identified in this transcript." message
- No topics section rendered anywhere
</success_criteria>
