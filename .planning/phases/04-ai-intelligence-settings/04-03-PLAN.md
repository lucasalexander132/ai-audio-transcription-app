...
<task type="auto">
  ...
  <action>
    ...
    3. In `transcribeChunk` handler:
       ...
       - Simplest: Create an internalQuery `getTranscriptUserId` in transcripts.ts that takes transcriptId and returns userId. Then in deepgram.ts actions, get userId from that, then get settings.
